...
- :func:`matchgate_inverse_channel_majorana_moments` -- the closed-form
  ...
  products, returning empirical means and certified Hoeffding radii.
...
def matchgate_inverse_channel_majorana_moments(
    ...
    *,
    ...
    # Precompute outcome covariances per shot (small: 2n x 2n each).
    ...
    for S in targets:
        ...
        shadow_norm_sq = norm
        shadow_norm = sqrt(shadow_norm_sq)

        # Per-shot estimator.
        per_shot = np.empty(M, dtype=np.complex128)
        for t in range(M):
            Q = record.rotations[t]
            # Schrödinger evolution: U_Q gamma_a U_Q^dagger = sum_j Q[j, a] gamma_j.
            # Our apply_matchgate_rotation_to_majorana_product uses the row
            # convention gamma_i -> sum_j Q[i, j] gamma_j, so passing Q^T gives
            # the Schrödinger column form.
            rotated = apply_matchgate_rotation_to_majorana_product(Q.T, S)
            acc = 0.0 + 0j
            cov_b = bit_covs[t]
            for S_prime, coeff in rotated.items():
                if not S_prime:
                    acc += coeff  # identity term: <b|I|b> = 1
                    continue
        ...
        # Sanity check: the per-shot estimator should be bounded by shadow_norm.
        # We don't enforce it strictly (numerical noise can push slightly past)
        # but include the shadow-norm value for use by downstream wiring.
        _ = fermionic_shadow_norm_pfaffian(S, n_modes=n)  # validates indices
